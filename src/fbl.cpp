...
float similarityWeight(const Vec3f & oValue, const Vec3f & cValue, float sigma) {
  float norm = sqrt(pow((oValue[0]-cValue[0]),2)+pow((oValue[1]-cValue[1]),2)+pow((oValue[2]-cValue[2]),2));
  return gaussianValue(norm, sigma);
}

Mat computeFBL(const Mat & img, const Mat & isophote, const float sigmaE, const float sigmaG, const float sigmaRE, const float sigmaRG, const int maxIter) {
  ...
  while (iteration) {
    ...
    std::cout << "FBL pass n°" << maxIter-iteration+1 << "/" << maxIter << std::endl;
    // first isophote way
