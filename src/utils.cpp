				  ...
				  const int c)
{
	...
	for (int i = -hMiddleKernel; i <= hMiddleKernel; i++) {
		for (int j = -vMiddleKernel; j <= vMiddleKernel; j++) {
			if (((i + r) >= 0 && (i + r) < pic.rows) && ((j + c) >= 0 &&
			 											 (j + c) < pic.cols))
			{
				conv += pic.at<float>(i + r, j + c) *
						kernel.at<float>(i + hMiddleKernel, j + vMiddleKernel);
			}
		}
	}
	return conv;
...
bool isZeroCrossPixel(Mat& img, int y, int x, float threshold) {
	...
	for (size_t i = 0; i < 4; i++) {
		if (neighboring[i][0] * neighboring[i][1]<0 &&
			abs(neighboring[i][0] - neighboring[i][1])>threshold)
		{
			return true;
		}
	}
...
Mat zeroCrossingMat(Mat& img, float threshold) {
	...
	for (int i = 1; i < (img.rows) - 1; i++) {
		for (int j = 1; j < (img.cols) - 1; j++) {
			zeroCross.at<uchar>(i, j) = isZeroCrossPixel(img, i, j, threshold)?\
			 							255 : 0;
		}
	}
	return zeroCross;
...
Mat thresholdUpperValues(const Mat& img, const float thresholdPerc) {
	...
	for (int i = 0; i < img.rows; i++) {
		for (int j = 0; j < img.cols; j++) {
			imgThresholded.at<uchar>(i, j) = (img.at<float>(i, j) > threshold)?\
			 								 255 : 0;
		}
	}
...
Mat Quantification(const Mat& src, int k){
	...
	float pas(255.0 / (float)k);
	for (int i= 0; i < (src.rows); i++) {
		for (int j = 0; j < (src.cols); j++) {
			quantif.at<Vec3b>(i,j)[0] = round((float)src.at<Vec3b>(i,j)[0]/pas)*
										pas;
			quantif.at<Vec3b>(i,j)[1] = round((float)src.at<Vec3b>(i,j)[1]/pas)*
										pas;
			quantif.at<Vec3b>(i,j)[2] = round((float)src.at<Vec3b>(i,j)[2]/pas)*
										pas;
