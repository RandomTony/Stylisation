              ...
              Mat& tKernelValues)
{
    ...
    Point2f gradient(-isophote.at<float>(lxsT.y, lxsT.x * 2 + 1),
    isophote.at<float>(lxsT.y, lxsT.x * 2 + 0));
    float result = 0.0;
    int tx, ty;
    ...
    for (int t = -tRadius; t <= tRadius; t++) {
        ...
        if (ty > -1 && ty < grayImg.rows && tx > -1 && tx < grayImg.cols) {
            result += tKernelValues.at<float>(0, t+tRadius) *
                      grayImg.at<float>(ty, tx);
        }
    }
...
Mat fDoG(const Mat& img, const Mat& isophote, const float sigmaM,
         const float sigmaC, const float rho, const float threshold,
         int maxIter)
{
    int cx, cy;
    ...
    for(int i = -tRadius; i <= tRadius; i++){
        tKernelValues.at<float>(0, i+tRadius) = gaussianValue(i, sigmaC) -
                                                rho*gaussianValue(i, sigmaS);
    }

    while (iteration) {
        std::cout << "fDoG: " << "pass n°" << (maxIter-iteration)+1 << "/" <<
                      maxIter << std::endl;
        ...
        std::cout << "Compute DoG along gradient foreach pixel..." <<
                     std::flush;
        // first pass, compute DoG at each pixel in the gradient
        for (int y = 0; y < grayImg.rows; y++) {
            for (int x = 0; x < grayImg.cols; x++) {
                lxct = Point2f(x, y);
                dog.at<float>(y, x) = lineDoG(grayImg, isophote, lxct,
                                              tKernelValues);
            }
        }
                ...
                for (float sign = -1; sign <= 1; sign+=2) { // Go in one way on the curve and on the other way with sign value
                    ...
                    for (int c = 1; c <= sRadius; c++) {
                            ...
                            cx < isophote.cols)
                        {
                            ...
                            hX += sKernelValues.at<float>(0,c+sRadius) *
                                  dog.at<float>(cy,cx);
