           ...
           float* gAmplitude, float* iAmplitude)
{
    ...
    *gAmplitude = (tensor.at<float>(0, 0)*tensor.at<float>(1, 1) + sqrt(delta))
                  / 2.0;
    *iAmplitude = (tensor.at<float>(0, 0)*tensor.at<float>(1, 1) - sqrt(delta))
                  / 2.0;
...
double coherenceNorm(const float gAmplitude, const float iAmplitude) {
    return pow((gAmplitude - iAmplitude) / (gAmplitude + iAmplitude), 2);
}

double dpX(const float gAmplitude, const float iAmplitude,
